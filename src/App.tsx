...
const SENT_COLOR: Record<string, string> = {
  Positive: "#3b6d11", Neutral: "#854f0b", Negative: "#a32d2d"
};

interface NewsItem { title: string; publisher: string; time: string; }
interface AIResult {
  price: number; changePercent: number;
  signal: string; confidence: string; priceTarget: string;
  technicalSummary: string; newsSentiment: string;
  newsSummary: string; reasoning: string; keyRisks: string;
  news: NewsItem[];
}
...
async function analyzeWithSearch(ticker: string): Promise<AIResult> {
...
function StockCard({ ticker, onRemove, autoRefreshInterval }: { ticker: string; onRemove: () => void; autoRefreshInterval: number }) {
  ...
  const analyze = useCallback(async () => {
    setStatus("loading"); setError(null); setTab("analysis");
    try {
      const ai = await analyzeWithSearch(ticker);
      setData(ai); setStatus("done");
    ...
    <div style={{ background: "#fff", border: "1px solid #e0dfd8", borderRadius: 12, overflow: "hidden", marginBottom: 12 }}>
      <div style={{ padding: "0.85rem 1.25rem", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, flexWrap: "wrap", borderBottom: status !== "idle" ? "1px solid #e0dfd8" : "none" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          {status === "done" && data && (
            <>
              <span style={{ fontSize: 15 }}>${data.price?.toFixed(2)}</span>
              <span style={{ fontSize: 13, color: data.changePercent >= 0 ? "#3b6d11" : "#a32d2d", fontWeight: 500 }}>
        ...
        <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
          {status === "done" && data && <SignalBadge signal={data.signal} />}
          ...
          <div style={{ display: "flex", gap: 6 }}>
            {["analysis", "news"].map(t => (
              <button key={t} onClick={() => setTab(t)} style={{
                ...
                {t === "analysis" ? "📊 Análisis" : `📰 Noticias (${data.news?.length || 0})`}
              </button>
            ))}
          </div>
          ...
          {tab === "analysis" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              <div style={{ display: "flex", gap: 14, fontSize: 12, color: "#666", flexWrap: "wrap" }}>
                <span>Confianza <b style={{ color: "#1a1a1a" }}>{data.confidence}</b></span>
                <span>Noticias <b style={{ color: SENT_COLOR[data.newsSentiment] }}>{data.newsSentiment}</b></span>
                {data.priceTarget && <span>Target <b style={{ color: "#1a1a1a" }}>{data.priceTarget}</b></span>}
              </div>
              <div>
                <div style={{ fontSize: 11, fontWeight: 600, color: "#999", textTransform: "uppercase", marginBottom: 3 }}>Precio y momentum</div>
                <div style={{ fontSize: 13, lineHeight: 1.6 }}>{data.technicalSummary}</div>
              </div>
              <div>
                <div style={{ fontSize: 11, fontWeight: 600, color: "#999", textTransform: "uppercase", marginBottom: 3 }}>Noticias</div>
                <div style={{ fontSize: 13, lineHeight: 1.6 }}>{data.newsSummary}</div>
              </div>
              <div style={{ background: "#f5f5f0", borderRadius: 8, padding: "8px 12px", fontSize: 13, lineHeight: 1.6 }}>
                <b>Conclusión:</b> {data.reasoning}
              </div>
              <div style={{ fontSize: 12, color: "#666", background: "#f5f5f0", borderRadius: 6, padding: "6px 10px" }}>⚠ Riesgo: {data.keyRisks}</div>
